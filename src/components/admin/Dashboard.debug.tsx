...
const Dashboard = () => {
  ...
  // Ajouter un log pour déboguer les projets
  useEffect(() => {
    console.log('Projets disponibles:', projects);
    if (projects.length > 0) {
      console.log('Premier projet:', projects[0]);
      console.log('Format de date:', projects[0].createdAt);
    }
  }, [projects]);
  ...
  const chartData = useMemo(() => {
    ...
    // Créer un Map pour un accès rapide aux mois
    const monthsMap = new Map(
      last6Months.map(item => [item.month, item])
    );

    console.log('Mois disponibles dans le graphique:', Array.from(monthsMap.keys()));

    // Compter les projets par mois
    projects.forEach(project => {
      // Vérifier si createdAt existe
      if (!project.createdAt) {
        console.log('Projet sans date de création:', project);
        return;
      }
      
      // Gérer différents formats de timestamp (serveur ou client)
      let projectDate;
      try {
        if (project.createdAt.seconds) {
          // Format Timestamp de Firestore
          projectDate = new Date(project.createdAt.seconds * 1000);
          console.log('Date du projet (format seconds):', projectDate);
        } else if (project.createdAt.toDate) {
          // Format serverTimestamp
          projectDate = project.createdAt.toDate();
          console.log('Date du projet (format toDate):', projectDate);
        } else if (project.createdAt instanceof Date) {
          // Format Date standard
          projectDate = project.createdAt;
          console.log('Date du projet (format Date):', projectDate);
        } else {
          // Format inconnu, essayer de convertir directement
          projectDate = new Date(project.createdAt);
          console.log('Date du projet (conversion directe):', projectDate);
        }
      } catch (error) {
        console.error('Erreur lors de la conversion de la date:', error, project.createdAt);
        return;
      }

      if (!projectDate || isNaN(projectDate.getTime())) {
        console.log('Date invalide après conversion:', projectDate, project.createdAt);
        return;
      }

      const monthKey = projectDate.toLocaleDateString('fr-FR', { 
        month: 'long',
        year: 'numeric'
      });
      console.log('Clé du mois pour le projet:', monthKey);

      const monthData = monthsMap.get(monthKey);
      if (monthData) {
        console.log('Mois trouvé dans le graphique, incrémentation des compteurs');
        monthData.total++;
        ...
        console.log('Mois non trouvé dans le graphique:', monthKey);
      }
    });

    const result = Array.from(monthsMap.values());
    console.log('Données finales du graphique:', result);
    return result;
  }, [projects]);
